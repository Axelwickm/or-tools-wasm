import type { SharedCaseMetadata } from '../../../harness/shared_case.ts';
import {
...
export const cpSatHighLevelParityCases: CpSatHighLevelParityCase[] = [
  ...
  },
  {
    ...
    async run({ CpModel, CpSolver, CpSolverSolutionCallback }) {
      ...
      const status = await solver.solve(model, { solutionCallback: solutionCounter });
      assertEqual(solver.statusName(status), 'OPTIMAL', `${this.name} status`);
  ...
  },
  {
    ...
    async run({ CpModel, CpSolver, CpSolverSolutionCallback }) {
      ...
      const status = await solver.solve(model, { solutionCallback: solutionObj });
      assertEqual(solver.statusName(status), 'OPTIMAL', `${this.name} status`);
